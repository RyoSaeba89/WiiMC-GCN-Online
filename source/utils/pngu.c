...
static u8 * PNGU_DecodeTo4x4RGBA8 (IMGCTX ctx, u32 width, u32 height, int * dstWidth, int * dstHeight, u8 *dstPtr)
{
	...
	if(padWidth%4) padWidth += (0-padWidth%4); //crops instead of adding
...
static u8 * PNGU_DecodeTo2x2RGB5A3 (IMGCTX ctx, u32 width, u32 height, int * dstWidth, int * dstHeight, u8 *dstPtr)
{
	u8 default_alpha = 255;
	u8 *dst;
	int x, y, x2, y2, offset;
...
// WORK
	...
	for(iv = 0, y1 = 0; y1 < (u32)padHeight; y1 += 4)
	{
		for(x1 = 0; x1 < (u32)padWidth; x1 += 4)
		{
			for(y = y1; y < (y1 + 4); y++)
			{
				for(x = x1; x < (x1 + 4); x++)
				{
					if((x >= padWidth) || (y >= padHeight))
						continue;
...
u8 * DecodePNG(const u8 *src, int * width, int * height, u8 *dstPtr)
{
	...
	if(PNGU_GetImageProperties(ctx, &imgProp) == PNGU_OK)
		...
		//dst = PNGU_DecodeTo2x2RGB5A3 (ctx, imgProp.imgWidth, imgProp.imgHeight, width, height, dstPtr);
