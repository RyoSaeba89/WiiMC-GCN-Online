...
BROWSERENTRY *VideoPlaylistGetNextShuffle()
{
	if(shuffleIndex_vid == -1 || shuffleIndex_vid >= browserVideos.numEntries)
	{
		...
		free(shuffleList_vid);
		shuffleList_vid = (int *)malloc(num*sizeof(int));
...
#if 0
	if(menuMode == 0 && menuCurrent == MENU_BROWSE_MUSIC && WiiSettings.autoPlayNextVideo)
	{
		// Continuous: If the last video is over, start from the first video
			if(WiiSettings.autoPlayNextVideo == AUTOPLAY_CONTINUOUS && browserMusic.selIndex == NULL)
				browserMusic.selIndex = browserMusic.first;

			if(WiiSettings.autoPlayNextVideo != AUTOPLAY_SHUFFLE)
				strcpy(loadedFile, browserMusic.selIndex->file);
			browserMusic.selIndex = WiiSettings.autoPlayNextVideo == AUTOPLAY_SHUFFLE ?
						MusicPlaylistGetNextShuffle() : browserMusic.selIndex->next;
...
bool InitMPlayer()
{
	...
	if(subtitleFontFound) {
		...
		font_mem = (u8 *)memalign(32, font_mem_size);
	...
	if(have_mono) {
		...
		mono_mem = (u8 *)memalign(32, mono_mem_size);
...
//	else
	if (WiiSettings.audioNorm == 1)
		...
		wiiSetKaraoke();
	else if (WiiSettings.audioNorm == 3)
		wiiSetRadio();
	else if (WiiSettings.audioNorm == 4)
		wiiSetMono();
}
...
#else
	GX_SetScreenPos(WiiSettings.videoXshift, WiiSettings.videoYshift, 
		WiiSettings.videoFull ? WiiSettings.videoZoomHor * 1.335
...
//	wiiSetVolNorm(); // now applied on every file load
...
//#if defined(HW_DOL)
	...
	//if(ShutdownRequested || WiiSettings.exitAction == EXIT_POWEROFF)
		//SYS_ResetSystem(SYS_POWEROFF, 0, 0);
	//if(WiiSettings.exitAction == EXIT_WIIMENU)
	exit(0);
}
