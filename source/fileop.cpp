...
//static DISC_INTERFACE* sp2 = &__io_sdsp2;
//static DISC_INTERFACE* gca = &__io_carda;
//static DISC_INTERFACE* gcb = &__io_cardb;
#endif
...
static void AddPartition(sec_t sector, int device, int type, int *devnum)
{
	...
	{
		case T_FAT:
			if(!fatMount(mount, disc, sector, 4, 64))
				return;
			sprintf(part[device][*devnum].name, "GCN");
...
#else
	if(device != DEVICE_SD && device != DEVICE_SP2)
#endif
...
#else
	if(devnum < 1 || devnum > 9 || (device != DEVICE_SD && device != DEVICE_SP2))
#endif
		return false;
...
static bool ParseDirEntries()
{
	...
	while(i < 20 && !parseHalt)
	{
		...
		if(f_entry)
		{
			f_entry->file = strdup(entry->d_name);
			...
			//	GetDisplay(f_entry, art_disp, 256);
				...
				f_entry->xml = strdup(entry->d_name);
				f_entry->image = strdup(entry->d_name);
...
ParseDirectory(bool waitParse)
{
	...
	f_entry->display = strdup(gettext(" .."));
	if(!f_entry->display) // no mem
	{
		DeleteEntryFiles(f_entry);
		return 0;
...
{
	char *buffer = (char*)malloc(MAX_PLX_SIZE);
	...
	f_entry->file = strdup(BrowserHistoryRetrieve());
	if(!f_entry->file) // no mem
	{
		DeleteEntryFiles(f_entry);
		free(list);
	...
	for(int i=0; i < numEntries; i++)
	{
		...
		if(list[i].url)
		{
			f_entry->file = strdup(list[i].url);
			if(!f_entry->file) // no mem
			{
				DeleteEntryFiles(f_entry);
				free(list);
		...
		if(list[i].name)
		{
			f_entry->display = strdup(list[i].name);
			if(!f_entry->display) // no mem
			{
				DeleteEntryFiles(f_entry);
				free(list);
		...
		if(list[i].thumb)
		{
			f_entry->image = strdup(list[i].thumb);
		...
		if(list[i].tunein)
		{
			f_entry->tunein = strdup(list[i].tunein);
...
nomemParsePlaylistFile:		
		...
		if(i->params) 
		{
			for (int n = 0; i->params[n].name != NULL; n++) 
			{
				...
				if(i->params[n].value == NULL)
					break;
				f_entry->display = strdup(i->params[n].value);
				if(!f_entry->display) // no mem
				{
					DeleteEntryFiles(f_entry);
					goto nomemParsePlaylistFile;
				...
				f_entry->image = strdup(i->params[n].image);
			//	f_entry->xml = strdup("sd1:/apps/wiimc/The Road to El Dorado.xml", BROWSER);
				break;
			}
		}
		...
		if(!f_entry->display)
		{
			...
			{
				start++;
				f_entry->display = strdup(start);
			}
			else
			{
				f_entry->display = strdup(i->files[0]);
			}
			if(!f_entry->display) // no mem
			{
				DeleteEntryFiles(f_entry);
...
//	if(browserOnlineMedia.first == NULL)
	...
	while(om_entry)
	{
		...
		if(strcmp(browser.dir, om_entry->file) == 0)
		{
			...
			if(om_entry->display)
			{
			f_entry->display = strdup(om_entry->display);
			...
			if(om_entry->image)
			{
			f_entry->image = strdup(om_entry->image);
			...
			if(om_entry->tunein)
			{
			f_entry->tunein = strdup(om_entry->tunein);
			...
			strncmp(browser.dir, om_entry->file, dirLen) == 0)
		{
			...
			if(!matchFound)
			{
				...
				f_entry->file = strdup(folder);
				if(f_entry->file == NULL) //no mem
				{
					DeleteEntryFiles(f_entry);
					break;
				}
				f_entry->display = strdup(folder);
				if(f_entry->display == NULL) //no mem
				{
					DeleteEntryFiles(f_entry);
					break;
...
#if 0
		// Assume ARAM
		if(buffersize == 880*1024) {

			char *tmp = (char*)realloc(buffer, loadSize);
			if(tmp == NULL) {
				free(buffer);
				printf("REALLOC FAIL,,");
				return 0;
				...
				buffer = tmp;
				readsize = fread (buffer, 1, loadSize, file);
			}
			readsize = fread (buffer, 1, loadSize, file);
		}
		else
