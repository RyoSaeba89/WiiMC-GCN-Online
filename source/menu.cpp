...
u32 bg_blue_jpg_size = 0x6C8E;
u32 bg_green_jpg_size = 0x70A1;
u32 bg_red_jpg_size = 0x628F;
u32 bg_jpg_size = 0x4689;

u32	font_ttf_size = 0x8450;
...
//	struct hostent *host = NULL;//net_gethostbyname(domain);
...
static bool safeFadeout = false;
static bool skipTrack = false;

static bool creditsActive = false;
...
unsigned bannerSSactive = 0;
unsigned ssIsActive = 0; // for tiled rendering in ss
...
static void *ScreensaverThread(void *arg)
{
	...
	GuiImageData logoLarge(logo_png);
	GuiImage logoLargeImg(&logoLarge);
	//if(!WiiSettings.screensaverArt) {
		logoLargeImg.SetScale(348, 72);
		logoLargeImg.SetPosition(0, -20);
	//}
	...
	while(1)
	{
		...
		for(int i=1; i < 4; i++)
			audiobarNowPlaying[i]->SetMaxWidth(screenwidth-100);
		...
		if(thumbImg != NULL && audiobarNowPlayingBtn->IsVisible() && WiiSettings.screensaverArt && WiiSettings.artwork) {
		...
		if(menuCurrent == MENU_BROWSE_VIDEOS && WiiSettings.bannerLimit != 0 && WiiSettings.artwork) {
		...
		//	printf("SHOWTEXT: %d,,", audiobarNowPlaying[1]->GetTextWidth());
...
*/
			...
			if(correctss && WiiSettings.screensaverArt == ART_TOP) {
				while(x < 30 || x > (screenwidth-max-30))
					x = (int)(((double)rand() / double(RAND_MAX + 1.0)) * screenwidth);
				while(y < thumbHeight-40 || y > (screenheight-w.GetHeight()))
					y = (int)(((double)rand() / double(RAND_MAX + 1.0)) * screenheight);
			} else if(correctss && WiiSettings.screensaverArt == ART_SIDE) {
				while(x < 216 || x > (screenwidth-max-30))
					x = (int)(((double)rand() / double(RAND_MAX + 1.0)) * screenwidth);
				while(y < thumbHeight-110 || y > (screenheight-w.GetHeight()))
					y = (int)(((double)rand() / double(RAND_MAX + 1.0)) * screenheight);
			} else {
				while(x < 30 || x > (screenwidth-w.GetWidth()-30))
					x = (int)(((double)rand() / double(RAND_MAX + 1.0)) * screenwidth);
			...
			//x = (screenwidth-max-30);
			//x = 29;
			//y = screenheight-w.GetHeight();
	...
	/*		if(WiiSettings.inactivityShutdown > 0 && 
		...
		if(creditsThreadHalt == 0)
			creditsThreadHalt = 1;
		if(WiiSettings.artwork || WiiSettings.bannerLimit != 0) {
...
static void *GuiThread (void *arg)
{
	...
	//	DoRumble(0);
		...
		if(mainWindow->GetState() != STATE_DISABLED)
		{
			if(wiiAudioOnly() && userInput[0].pad.btns_h & PAD_BUTTON_X &&
				(userInput[0].pad.btns_d & PAD_BUTTON_DOWN || userInput[0].pad.btns_d & PAD_BUTTON_UP))
			{
				wiiPause();
			}
			else if(wiiAudioOnly() && userInput[0].pad.btns_h & PAD_BUTTON_X && userInput[0].pad.btns_d & PAD_TRIGGER_R)
			{
				// Doing this lags frame generation
			//	StopMPlayerFile(); // end this song
			//	while(controlledbygui == 2) // wait for song to end
			//		usleep(THREAD_SLEEP);
			//	FindNextFile(true); // find next song
			...
			//		thumbImg->SetImage(NULL);
			//	}
			//	if(thumbImg->GetEffect() == EFFECT_FADE) {
			//		thumbImg->SetVisible(false);
			//		thumbImg->SetImage(NULL);
			//	}
	...
	// test various forced inputs
	if(wiiAudioOnly() && userInput[0].pad.btns_h & PAD_BUTTON_X) {
		if(userInput[0].pad.btns_d & PAD_BUTTON_RIGHT) {
			wiiSync(WiiSettings.skipForward);
		} else if(userInput[0].pad.btns_d & PAD_BUTTON_LEFT) {
			wiiSync(-WiiSettings.skipBackward);
		}
	}
		  ...
		  (!screensaverThreadHalt && menuCurrent == MENU_BROWSE_VIDEOS && WiiSettings.bannerLimit != 0 &&
			...
			//	}
				...
				for (int x = 0; x < 2; x++)
				{
					...
					GX_CopyDisp((void *)((u32)xfb[whichfb] + xfb_offset), GX_TRUE);
				#else
					GX_CopyDisp((void *)((u32)xfb[0] + xfb_offset), GX_TRUE);
				#endif
				}
			}
			doneSafe = true;
		...
		{
			if(ssTimer != 0 && (!banner_pic || !embedded_pic))
			{
				ssTimer = 0;
		...
		if(userInput[0].pad.btns_d & PAD_TRIGGER_Z && !ssIsActive && !showProgress && menuCurrent < 2)
			ResumeCreditsThread();
...
restart:
		...
		if(file)
		{
			...
			if(ext_font_ttf)
			{
				...
				{
					sprintf(error, "Could not change language. Font file corrupted!");
				}
			}
			else
			{
...
WindowPrompt(const char *title, wchar_t *msg, const char *btn1Label, const char *btn2Label)
{
	...
	GuiImageData btnOutlineOver(NULL);

	GuiImageData dialogBox(dialogue_box_png);
	GuiImage dialogBoxImg(&dialogBox);
	dialogBoxImg.SetScale(18);
...
ProgressWindow(char *title, char *msg)
{
	...
	GuiText cancelBtnTxt("Return", 20, (GXColor){255, 255, 255, 255});
...
{
	return WindowPrompt("Error", gettext(msg), "Retry", "Return");
}
...
{
	return WindowPrompt("Error", msg, "Retry", "Return");
}
...
static void *ThumbThread (void *arg)
{
	...
	// A: Use ARAM to break the file limit
	// B: Allocate lowest mem possible and realloc when loading banners
	// Ideally, I want A...
#if defined(HW_DOL)
//	char *thumbBuffer = (char *)VM_Init(AR_GetSize(), 256*1024);
//	char *thumbBuffer = (char *)malloc(880*1024);
	char *thumbBuffer = NULL;
#else
//	char *thumbBuffer = (char *)malloc(880*1024);
	char *thumbBuffer = NULL;
#endif
	...
	char *upArt = (char *)malloc(1024*8);
	char *dlBNR = (char *)malloc(4);

	// built-in sites, anison.fm max size is unknown.
	char *upArt_hard = (char *)malloc(1024*8);
#endif
	...
	//	if(thumbBuffer && thumbLoad)
		if(thumbLoad)
		{
			...
			//if(!WiiSettings.artworkFade || !screensaverThreadHalt) {
			if(!WiiSettings.artworkFade || menuCurrent == MENU_BROWSE_VIDEOS) {
				thumbImg->SetVisible(false);
				thumbImg->SetImage(NULL);
			}
			ResumeGui();
			...
			{
		#if 1
			//	if(WiiSettings.artworkFade != 0 && screensaverThreadHalt) {
				if(WiiSettings.artworkFade != 0 && menuCurrent == MENU_BROWSE_MUSIC) {
					thumbImg->SetEffect(EFFECT_FADE, -WiiSettings.artworkFade);
					//FADE1
					// If the images load fast enough, the fade is useless
					do {
						// Might help avoid lag when loading a new file
						//usleep(200);
			...
			if(thumbIndex || embedded_pic || banner_pic)
			{
				...
				if(!bannerSSactive && loadIndex->image && strncmp(browser.selIndex->file, "http:", 5) == 0) {
					...
					if (WiiSettings.numThumb > 0) {
						...
						// PLX XMLs would leak otherwise
							free(WiiSettings.descTxt);
							free(WiiSettings.yearNum);
						}
					}
					loadIndex->year = strdup(year_txt);
					loadIndex->desc = strdup(desc_txt);
					free(loadIndex->year);
					free(loadIndex->desc);
					secure_type = false;
				...
				//		printf("THUMBS: %s", actual_path);
					...
					loadIndex->year = strdup(year_txt);
					loadIndex->desc = strdup(desc_txt);
					loadIndex->image = strdup(actual_path);
				...
				//		}
						...
						if(!WiiSettings.onlineBanners) {
							...
							#endif
								thumbBuffer = (char *)malloc(fsize);
								
								read = LoadFile(thumbBuffer, fsize+4, banner_path, SILENT);
				...
				if((read > 0 && loadIndex == thumbIndex) || (pos_pic && embedded_pic) || (read > 0 && banner_pic)) // file loaded and index has not changed
				{
					if(!embedded_pic) {
						...
						thumb = new GuiImageData((uint8_t *)pos_pic, epic_size, GX_TF_RGBA8);
						if(pos_pic) {
							free(pos_pic);
							pos_pic = NULL;
						}
			...
			//++noArtCNT;
		#if 1 // Fadeout after a song ends, overpowers other fadeout,
			// the problem is that it should happen AFTER a song has loaded
			// otherwise the fadeout can look laggy.
			// IF the next song has artwork, it should use the above fadeout,
			// while the fadeout below should only be for when there is no art or audio ends
			// but there's just no easy way to track all of that...

		#if 0
			// This idea only works if audio stops playing
			if(thumb && WiiSettings.artworkFade && menuCurrent == MENU_BROWSE_MUSIC && !wiiAudioOnly())
			{
				if(fadeTimer == 0)
					fadeTimer = gettime();
				if(diff_sec(fadeTimer, gettime()) > 2) {
					safeFadeout = true;
					fadeTimer = 0;
				}
			}
		#endif
			// Better idea: if no art is found, trigger safeFadeout,
			// this gives purpose to the 1st fadeout, and avoids the lag.
			// But now art will remain if audio ends. I'll take it.
			
		//	if(thumb && WiiSettings.artworkFade && menuCurrent == MENU_BROWSE_MUSIC && !wiiAudioOnly())
			if(thumb && WiiSettings.artworkFade && safeFadeout && menuCurrent == MENU_BROWSE_MUSIC)
		//	if(safeFadeout)
			{
			//	printf("FADE2 ACTIVE!");
			//	ExitRequested = true;
...
//	VM_InvalidateAll();
//	VM_Deinit();
	if(thumbBuffer) free(thumbBuffer);
#else
	if(thumbBuffer) free(thumbBuffer);
#endif
#ifdef USE_DL_ART
...
static int LoadNewFile()
{
	...
	// remove cover art if new file is loading, no need to support fade out
	if(thumbImg != NULL && menuCurrent == MENU_BROWSE_MUSIC) {
		if(!WiiSettings.artworkFade) {
			thumbImg->SetVisible(false);
			thumbImg->SetImage(NULL);
		}
	}
...
//	playlistResetBtn.SetTrigger(&trigMinus);
	playlistResetBtn.SetTrigger(NULL);
	...
	if(menu == MENU_BROWSE_ONLINEMEDIA)
	{
		if(!browserOnlineMedia.first)
		{
			...
			if(stat(filepath, &buf) == 0)
				ErrorPrompt("Online media invalid.");
			//else
...
#else
	if ((menu == MENU_BROWSE_VIDEOS || menu == MENU_BROWSE_MUSIC) && WiiSettings.artwork) {
#endif
		thumbImg = new GuiImage;
		thumbImg->SetAlignment(ALIGN_RIGHT, ALIGN_TOP);
		thumbImg->SetVisible(false);
		...
		for(int i=0; i<pagesize; i++)
		{
			if(fileBrowser->fileList[i]->GetState() == STATE_CLICKED)
			{
				...
				if(browser.selIndex->type == TYPE_SEARCH)
				{
					...
					free(browser.selIndex->file);
					browser.selIndex->file = strdup(tmp);
				}
#endif
				// this is a file
				char ext[7];
...
#if 0 // 0 if you want cover art to persist
...
//	sprintf(options.name[i++], "Art Folder");
	...
	sprintf(options.name[i++], "VI Width");
	...
	while(menuCurrent == MENU_SETTINGS_GLOBAL && !guiShutdown)
	{
		...
		switch (ret)
		{
			...
			case 5:
				...
				if(WiiSettings.startArea > MENU_BROWSE_MUSIC)
		...
		//		break;
			...
			//	sprintf(limit, "%04d", WiiSettings.bannerLimit);
				// Require the artwork viewer to enable
				if(!WiiSettings.artwork)
					InfoPrompt("Artwork Viewer must be on to use this.");
				else if(WiiSettings.bannerFolder[0] == 0)
					InfoPrompt("Banner folder must be set first.");
				else {
					if(WiiSettings.bannerLimit)
						WiiSettings.bannerLimit = 0;
					else
				...
				break;
			case 8:
				if(WiiSettings.bannerFolder[0] == 0)
					strcpy(WiiSettings.bannerFolder, "sd1:/");
				
				OnScreenKeyboard(WiiSettings.bannerFolder, MAXPATHLEN);
				CleanupPath(WiiSettings.bannerFolder);
				
				if(WiiSettings.bannerFolder[5] == 0) {
...
#endif
			sprintf(options.value[3], "%d %s", (int)(WiiSettings.screensaverDelay/60), gettext("min"));
#if 0
			if(WiiSettings.inactivityShutdown > 0)
...
#endif		
			sprintf(options.value[4], "%s", WiiSettings.lockFolders ? "Static" : "Use Last Browsed");
			
			switch(WiiSettings.startArea)
			{
				case MENU_BROWSE_VIDEOS: 		sprintf(options.value[5], "Playback"); break;
				case MENU_BROWSE_MUSIC: 		sprintf(options.value[5], "Music"); break;
				//case MENU_BROWSE_ONLINEMEDIA: 	sprintf(options.value[5], "Online Media"); break;
			}
			
			sprintf(options.value[6], "%s", WiiSettings.artwork ? "On" : "Off");
		//	snprintf(options.value[7], 60, "%s", WiiSettings.artworkFolder);
			sprintf(options.value[7], "Total: #%d", WiiSettings.bannerLimit);
			snprintf(options.value[8], 60, "%s", WiiSettings.bannerFolder);
			sprintf(options.value[9], "%s", WiiSettings.jpegQuality ? "On" : "Off");
			sprintf(options.value[10], "%s", WiiSettings.night ? "On" : "Off");
			sprintf(options.value[11], "%s", WiiSettings.videoDf ? "On" : "Off");
			sprintf(options.value[12], "%s", WiiSettings.viWidth ? "NTSC (720)" : "1:1 (704)");
		//	sprintf(options.value[16], "%s", WiiSettings.screenDim ? "On" : "Off");
			sprintf(options.value[13], "%s", WiiSettings.doubleStrike ? "On" : "Off");
		//	sprintf(options.value[13], "%s", WiiSettings.force576p ? "On" : "Off");
			//sprintf(options.value[16], "%s", WiiSettings.tiledRender ? "On" : "Off");
		#if 0
			switch(WiiSettings.tiledRender)
			{
				case 0:
					sprintf (options.value[14], "Off"); break;
				case 1:
					sprintf (options.value[14], "On"); break;
				case 2:
					sprintf (options.value[14], "Auto"); break;
			}
		#endif
...
//	sprintf(options.name[i++], "Auto-Play Next");
...
//	sprintf(options.name[i++], "Skip Deblocking Filter");
	...
	GuiText titleTxt("Settings - Playback", 28, (GXColor){255, 255, 255, 255});
	titleTxt.SetAlignment(ALIGN_LEFT, ALIGN_TOP);
	titleTxt.SetPosition(49, 100);
...
//	backBtn.SetImageOver(&backBtnImgOver);
	...
	while(menuCurrent == MENU_SETTINGS_VIDEOS && !guiShutdown)
	{
...
#endif
			sprintf (options.value[0], "%d%%", WiiSettings.cacheFill);
		...
		//	sprintf (options.value[7], "%s", WiiSettings.autoResume ? "On" : "Off");
			...
			{
				case AUTOPLAY_OFF:
					sprintf (options.value[1], "Off"); break;
				case AUTOPLAY_ON:
					sprintf (options.value[1], "Through"); break;
				case AUTOPLAY_SHUFFLE:
					sprintf (options.value[1], "Shuffle"); break;
				case AUTOPLAY_LOOP:
					sprintf (options.value[1], "Loop"); break;
				case AUTOPLAY_CONTINUOUS:
					sprintf (options.value[1], "Continuous"); break;
			}
		#endif
			sprintf (options.value[1], "%d %s", WiiSettings.skipBackward, gettext("sec"));
			sprintf (options.value[2], "%d %s", WiiSettings.skipForward, gettext("sec"));
		//	snprintf(options.value[3], 60, "%s", WiiSettings.videosFolder);
		//	sprintf (options.value[12], "%s", WiiSettings.videoFull ? "On" : "Off");
			//sprintf (options.value[3], "%d%", WiiSettings.audioNorm);
			switch(WiiSettings.audioNorm)
			{
				case 0:
					sprintf (options.value[3], "Off"); break;
				case 1:
					sprintf (options.value[3], "Volnorm"); break;
				case 2:
					sprintf (options.value[3], "Karaoke"); break;
				case 3:
					sprintf (options.value[3], "Radio"); break;
				case 4:
					sprintf (options.value[3], "Mono"); break;
			}
...
static void MenuSettingsMusic()
{
	...
	sprintf(options.name[i++], "Artwork Fade");
	...
	while(menuCurrent == MENU_SETTINGS_MUSIC && !guiShutdown)
	{
		...
		switch (ret)
		{
			...
			case 1:
				...
				if(WiiSettings.screensaverArt > ART_FULL_ALT)
					WiiSettings.screensaverArt = 0;
				if(WiiSettings.screensaverArt != 0)
					WiiSettings.artwork = 1;
				break;
			case 2:
				char limit[5];
				limit[0] = 0;
				if(OnScreenKeypad(limit, 2, true))
				{
					if(limit[0] == 0)
					{
					...
					{
						int result = atoi(limit);
						WiiSettings.artworkFade = result;
					}
				}
				break;
			case 3:
				...
				if(OnScreenKeypad(limit, 4, true))
				{
					...
					{
						int result = atoi(limit);
						WiiSettings.nativeLoops = result;
					}
				}
				break;
		#if 0
		...
		if(ret >= 0 || firstRun)
		{
			...
			if(WiiSettings.artworkFade)
				sprintf(options.value[2], "%d", WiiSettings.artworkFade);
			else
				sprintf(options.value[2], "No");
			
			if(WiiSettings.nativeLoops)
				sprintf(options.value[3], "%d", WiiSettings.nativeLoops);
			else
				sprintf(options.value[3], "No");
			snprintf(options.value[4], 60, "%s", WiiSettings.musicFolder);
...
//	backBtn.SetImageOver(&backBtnImgOver);
	...
	while(menuCurrent == MENU_SETTINGS_NETWORK && !guiShutdown)
	{
		...
		else if(ret >= 0 && map[ret] >= MAX_SHARES)
		{
...
static void MenuSettings()
{
	...
	// hmm, come here...
	// GuiOptionBrowser doesn't free certain elements
...
{
	return false;//videobarVolumeLevelBtn->IsVisible();
}
...
//	pointerImg[1] = player1_point_png;
//	pointerImg[2] = player1_point_png;
//	pointerImg[3] = player1_point_png;
	//pointerGrabImg[0] = NULL;
	//pointerGrabImg[1] = NULL;
	//pointerGrabImg[2] = NULL;
	//pointerGrabImg[3] = NULL;
...
//	volumeLine = new GuiImageData(volume_line_png);
	...
	bg->SetImage(bg_jpg, bg_jpg_size);
#else
	bg = new GuiImageData(bg_lo_jpg, GX_TF_CMPR);
	bg->SetImage(bg_lo_jpg, 0x9600);
#endif
	
	navDivider = new GuiImageData(nav_divider_lo_jpg);
	navDivider->SetImage(nav_divider_lo_jpg, 0x1B5);
	
	//btnBottom = new GuiImageData(NULL);
...
//	audiobarForwardTip->SetPosition(-1, 0);
	...
	audiobarModeBtn->SetPosition(300, 4);
...
//	settingsOver = new GuiImageData (nav_settings_over_png);
	...
	videosBtnTip = new GuiTooltip ("Playback");
