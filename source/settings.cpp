...
static const char * XMLSaveCallback(mxml_node_t *node, int where)
{
	...
	if (where == MXML_WS_BEFORE_OPEN)
	{
		...
		else if(!strcmp(name, "setting") || !strcmp(name, "smbshare"))
			return ("\n\t");
...
//	createXMLSetting("inactivityShutdown", "Inactivity shutdown", toStr(WiiSettings.inactivityShutdown));
	createXMLSetting("lockFolders", "Start folder", toStr(WiiSettings.lockFolders));
...
//	createXMLSetting("debug", "Debug", toStr(WiiSettings.debug));
	...
	createXMLSetting("bannerLimit", "Amount of banners found", toStr(WiiSettings.bannerLimit));
	createXMLSetting("bannerFolder", "Banner folder", WiiSettings.bannerFolder);
	createXMLSetting("jpegQuality", "JPEG lo-mem resample", toStr(WiiSettings.jpegQuality));
	createXMLSetting("night", "Night Filter", toStr(WiiSettings.night));
	createXMLSetting("videoDf", "Deflicker", toStr(WiiSettings.videoDf));
	createXMLSetting("viWidth", "Video Interface Width", toStr(WiiSettings.viWidth));
...
//	createXMLSetting("tiledRender", "Render More Pixels", toStr(WiiSettings.tiledRender));
	// Videos
	createXMLSection("Playback", "Playback Settings");
...
//	createXMLSetting("videoFull", "Videos in Fullscreen", toStr(WiiSettings.videoFull));
	createXMLSetting("audioNorm", "Filter", toStr(WiiSettings.audioNorm));
...
//	createXMLSetting("interlaceHandle", "How to handle interlacing", toStr(WiiSettings.interlaceHandle));
	...
	createXMLSetting("artworkFade", "Apply fade effect", toStr(WiiSettings.artworkFade));
...
static void RecurseThumbs(mxml_node_t * top, char * path)
{
	...
	while(next != NULL)
	{
		...
		if(number)
		{
			...
			WiiSettings.numThumb = atoi(number);
		//	printf("THUMB: %d", WiiSettings.numThumb);

			if(year)
			{
				WiiSettings.yearNum = strdup(year);
			}
			if(desc)
			{
				WiiSettings.descTxt = strdup(desc);
			}
...
//	WiiSettings.exitAction = EXIT_AUTO;
	...
	WiiSettings.screensaverDelay = 120;
	//WiiSettings.inactivityShutdown = 2;
	WiiSettings.lockFolders = 0;
	WiiSettings.startArea = MENU_BROWSE_MUSIC;
...
//	WiiSettings.interlaceHandle = 1; //let TV handle it
	...
	WiiSettings.screensaverArt = ART_TOP; // this also affects the banner ss, so enable it
...
static void FixInvalidSettings()
{
	...
	if(WiiSettings.lockFolders != 1 && WiiSettings.lockFolders != 0)
		WiiSettings.lockFolders = 0;
	if(WiiSettings.startArea < MENU_BROWSE_VIDEOS || WiiSettings.startArea > MENU_BROWSE_MUSIC)
...
//		WiiSettings.tiledRender = 0;
	...
	//	WiiSettings.videoFull = 0;
	if(WiiSettings.audioNorm < 0 || WiiSettings.audioNorm > 4)
		WiiSettings.audioNorm = 0;
	...
	if(WiiSettings.playOrder < 0 || WiiSettings.playOrder > PLAY_THROUGH)
		WiiSettings.playOrder = PLAY_SINGLE;
	if(WiiSettings.artworkFade < 0 || WiiSettings.artworkFade > 99)
		WiiSettings.artworkFade = 0;
...
//		WiiSettings.dvdDisabled = 0;
	...
	// Some quick fixes for release
	if(WiiSettings.artwork < 1 && WiiSettings.screensaverArt)
		WiiSettings.screensaverArt = 0;
	if(WiiSettings.screensaverArt > 0 && !WiiSettings.artwork)
		WiiSettings.artwork = 1;
	
	if(WiiSettings.bannerLimit > 0 && !WiiSettings.artwork)
		WiiSettings.bannerLimit = 0;
...
SaveSettings (bool silent)
{
	...
	else
	{
		...
		{
			if(!silent)
				ErrorPrompt("Could not find SD card - one is required for normal operation.");
			return false;
		}
	...
	if (offset > 0)
	{
		...
		if(xml)
		{
			...
			//	loadXMLSetting(&WiiSettings.interlaceHandle, "interlaceHandle");
				...
				loadXMLSetting(&WiiSettings.artworkFade, "artworkFade");
